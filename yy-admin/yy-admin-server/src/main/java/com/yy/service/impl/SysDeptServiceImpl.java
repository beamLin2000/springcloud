...
public class SysDeptServiceImpl extends BaseServiceImpl<SysDeptDao, SysDeptEntity> implements SysDeptService {
    ...
    public void update(SysDeptDTO dto) {
        ...
        if(entity.getId().equals(entity.getPid())){
            throw new YYException(ErrorCode.SUPERIOR_DEPT_ERROR);
        }
    ...
    public void delete(Long id) {
        ...
        if(subList.size() > 1){
            throw new YYException(ErrorCode.DEPT_SUB_DELETE_ERROR);
        }

        //判断部门下面是否有用户
        int count = sysUserDao.getCountByDeptId(id);
        if(count > 0){
            throw new YYException(ErrorCode.DEPT_USER_DELETE_ERROR);
        }

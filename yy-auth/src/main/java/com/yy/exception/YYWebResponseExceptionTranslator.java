 ...
 */
@Component
public class YYWebResponseExceptionTranslator implements WebResponseExceptionTranslator<OAuth2Exception> {
    ...
    public ResponseEntity<OAuth2Exception> translate(Exception e) {
        ...
        if (exception != null) {
            return handleOAuth2Exception(new YYOAuth2Exception(e.getMessage(), e));
        }

        exception = (AccessDeniedException) throwableAnalyzer.getFirstThrowableOfType(AccessDeniedException.class, causeChain);
        if (exception != null) {
            return handleOAuth2Exception(new YYOAuth2Exception(exception.getMessage(), exception));
        }
        ...
        return handleOAuth2Exception(new YYOAuth2Exception(HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), e));
    }
